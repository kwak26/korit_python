...
#id()
#객체를 입력받아서 고유 주소값(레퍼런스)를 리턴하느 ㄴ함수
a = 3
print(id(a))
...
#max()
#반복가능한 데이터 중에[서 최댓값을 리턴
num_list = [1, 2, 13, 15, 23, 18, 55, 17, 46]
print(max(num_list))

#min()
#반복가능한 데이터 중에서 최소값을 리턴
num_list = [1, 2, 13, 15, 23, 18, 55, 17, 46]
print(min(num_list))
...
# r 읽기 모드
# a 추가 모드
file = open("안녕하세요.txt", "r", encoding="utf-8")
content = file.read()
print(content)
file.close()

with open("안녕하세요.txt", "r", encoding="utf-8") as file:
    content = file.read()
    print(content)
...
#실습문제
data = ["apple", "banana", "cherry", "grape", "mango", "blueberry"]
# 1. apple
# 2. banana
